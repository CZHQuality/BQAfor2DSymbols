#This code is the training prodecure for Subtask 2 of industrial 2D symbols, i.e. Quality Grade Estimation
'''
Author: Zhaohui Che
...
n_classes = 5
dropout = 0.75 
n_classes = n_classes + 1 #this is for better restoring the trained model, and the categories of quality grades is still 5, just map level-4 to level-5, other levels keep unchangedd
...
init = tf.global_variables_initializer()
var_list = [weights['wc3'], weights['wc4'], weights['wd1'], weights['out'],  biases['bc3'], biases['bc4'], biases['bd1'], biases['out']]  
initfc = tf.variables_initializer(var_list, name='init')  #Attention!!! inherit the weights and bias of the shared layers, but initialize the rest layer for better training subtask2
...
with tf.Session() as sess:
    ...
    saver.restore(sess, "./MyModel/modelSubtask1.ckpt")   
    sess.run(initfc)  
    ...
    while count<20: #epoch total amout is 20
        count = count+1
        print("count:",count)
        for batch_id in range(0, 30):# training set for subtask2
    ...
    saver.save(sess,"./MyModel/modelSubtask2.ckpt")
